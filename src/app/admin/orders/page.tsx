...
function Dashboard() {
    ...
    filter === "All"
      ...
      const handleOrderDetails = (order: Order) => {
        Swal.fire({
          ...
          width: "50%", // Set width to 100% for a larger modal
          didOpen: () => {
            // Modify the height of the entire SweetAlert modal container
            const swalModal = document.querySelector('.swal2-modal') as HTMLElement;
            if (swalModal) {
              swalModal.style.maxHeight = '350px'; // Adjust this value as needed
              swalModal.style.overflowY = 'auto';  // Allow scrolling if the modal content exceeds height
            }
          },
  ...
  const handleStatusChange = async (
    ...
    e.stopPropagation(); // Prevent row click from triggering SweetAlert

    const newStatus = e.target.value;
    if (newStatus === "") return; // Do nothing if no valid status is selected
  ...
  const handleRowClick = (order: Order, e: React.MouseEvent) => {
    // Only show order details if the click isn't from the dropdown
      ...
      <div className="flex h-screen bg-white overflow-hidden font-serif">
        ...
        <div className="flex-1 flex flex-col ml-64">
          ...
          <div className="flex-1 p-2 overflow-y-auto">
            <div className="overflow-y-auto p-2 transition-all duration-300">
              <table className="w-full table-auto text-sm rounded-lg">
                ...
                <tbody className="divide-y divide-gray-200">
                  {filteredOrders.map((order) => (
                    <tr
                      key={order._id}
                      onClick={(e) => handleRowClick(order, e)} // Row click will trigger only if not coming from dropdown
                    ...
                    >
                      ...
                      <td className="py-4 px-2">
                        <select
                          ...
                          onChange={(e) => handleStatusChange(e, order._id)} // Show SweetAlert for status change
